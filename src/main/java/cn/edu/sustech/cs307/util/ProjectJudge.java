        ...
        for (File file : searchCourseDir.listFiles((dir, name) -> !name.endsWith("Result.json"))) {
            ...
            for (int i = 0; i < 100; i++){
                if (!searchCourseExpected.get(i).equals(searchCourseResult.get(i))){
                    System.out.print(i + ": " + Math.min(searchCourseExpected.get(i).size(),searchCourseResult.size()) + " ");
                    for (int j = 0; j < Math.min(searchCourseExpected.get(i).size(),searchCourseResult.get(i).size()); j++){
                        System.out.print(searchCourseExpected.get(i).get(j).equals(searchCourseResult.get(i).get(j)) + " ");
                    }
                }
            }
        ...
        for (File file : enrollCourseDir.listFiles((dir, name) -> !name.endsWith("Result.json"))) {
            ...
            for (int i = 0; i < enrollCourseParams.size(); i++) {
                ...
                if (expected == result) {
                    ...
                    System.out.println("The result is "+expected.toString());
